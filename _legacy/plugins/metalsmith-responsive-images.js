...
const {
  ...
  builtinDecode,
  drawableToImageData
} = require('../../frontend-image-encode/lib2')
            ...
            try {
              ...
              for (let i = 0; i < opts.format.length; i++) {
                ...
                for (let j = opts['sizes-attr'].length - 1; j >= 0; j--) {
                  if (
                    opts['sizes-attr'].length ===
                    imageSet[opts.format[i]].length
                  ) {
                ...
                if (opts.format[i] === 'jpg') {
                  ...
                  imageSetHtml +=
                    '" src="' +
                    (
                      metalsmith._metadata.rootpath + smallestImage.name
                    ).replace(/\/\//g, '/') +
                    '" alt="' +
                    p1 +
                    '" title="' +
              ...
              // Image is not resizeable but we assume it is still an image
              // (probabably a gif) so we load it anyway and make a simple
              // picture element
              const result = drawableToImageData(await builtinDecode(file))
              imageSetHtml +=
                '<img src="' +
                (metalsmith._metadata.rootpath + file.name).replace(
                  /\/\//g,
                  '/'
                ) +
                '" alt="' +
